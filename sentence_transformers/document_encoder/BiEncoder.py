...
class BiEncoder():
    ...
    def get_dimensions(array, level=0):
        ...
        try:
            for row in array:
                yield from BiEncoder.get_dimensions(row, level + 1)
    ...
    def get_max_shape(array):
        ...
        for level, length in BiEncoder.get_dimensions(array):
    ...
    def iterate_nested_array(array, index=()):
        try:
            for idx, row in enumerate(array):
                yield from BiEncoder.iterate_nested_array(row, (*index, idx))
        except TypeError:  # final level
            yield (*index, slice(len(array))), array

    @staticmethod
    def pad(array, fill_value):
        dimensions = BiEncoder.get_max_shape(array)
        result = np.full(dimensions, fill_value)
        for index, value in BiEncoder.iterate_nested_array(array):
            result[index] = value
        return result
    ...
    def fit(self,
        ...
        if use_fsdp:
            self.token_pooling_layer = FSDP(self.token_pooling_layer)
        if use_amp:
        ...
        # Prepare optimizers
        param_optimizer = list(self.token_pooling_layer.named_parameters())
        ...
        for epoch in trange(epochs, desc="Epoch"):
            training_steps = 0
            self.token_pooling_layer.zero_grad()
            self.token_pooling_layer.train()
