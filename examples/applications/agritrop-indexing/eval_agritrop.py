...
def create_evaluator(dataframe, device, text_field='sentence1', limit=-1):
    ...
    dataframe_new_column = dataframe.groupby(['doc_ids'])['concept_ids'].apply(','.join).reset_index().rename(
        columns={'concept_ids': 'merge_concept_ids'})
    ...
    for i in tqdm(range(0, len(dataframe_merge))):
        if dataframe_merge['split'][i] == 'dev':
            if dataframe_merge['score'][i] == 1:
                relevant_concept_ids_dev[dataframe_merge['doc_ids'][i]] = set(
                    dataframe_merge['merge_concept_ids'][i].split(","))
        if dataframe_merge['split'][i] == 'test':
            if dataframe_merge['score'][i] == 1:
                relevant_concept_ids_test[dataframe_merge['doc_ids'][i]] = set(
                    dataframe_merge['merge_concept_ids'][i].split(","))
    ...
    ir_evaluator_dev = evaluation.DocumentInformationRetrievalEvaluator(queries_dev, corpus_concept_ids_dev,
                                                                        relevant_concept_ids_dev,
    ...
    ir_evaluator_test = evaluation.DocumentInformationRetrievalEvaluator(queries_test, corpus_concept_ids_test,
                                                                         relevant_concept_ids_test,
                                                                         show_progress_bar=True,
                                                                         corpus_chunk_size=100000,
                                                                         precision_recall_at_k=[10, 100],
